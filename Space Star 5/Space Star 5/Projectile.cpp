...
void Missile1::Update(float dt)
{
	...
	m_rotateAngle = (m_rotateAngle == 360) ? 0 : m_rotateAngle += 1*dt;
...
//-----------------------------------------------------------------------------
/*
 * Add some stuff about the class
 *
 */
//------------------------------------------------------------------------------
...
BlastRadius::BlastRadius(D3DXVECTOR3 spawnPos, float scale)
{
	position = startPosition = spawnPos;
	this->direction = D3DXVECTOR3(0.0,0.0,0.0);
	D3DXMatrixScaling(&scaleMat, scale,scale,scale);
	D3DXMatrixRotationYawPitchRoll(&rotateMat, 0,0,0);
	isBlastMesh = isExpanding = true;
	projectileType = BLAST_RADIUS;
	destroyObject = false;
	blastRadiusScale = scale;
	radiusTimeTrack = 0.0;

	Initialize();
}

BlastRadius::~BlastRadius(void)
{
	Shutdown();
}

void BlastRadius::Initialize()
{
	// build bounding box for the mesh
	BYTE* vertices = NULL;
...
void BlastRadius::Render(ID3DXEffect* shader)
{
	// WVPMat - World View Projection Matrix
	// WITMat - World Inverse Transverse Matrix
	D3DXMATRIX WVPMat, WITMat;
